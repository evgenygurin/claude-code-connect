...
export class SessionManager extends EventEmitter {
  ...
  private async createPullRequest(
    ...
    try {
      ...
      // Create PR body with commits and files
      const prBody = `
## 🤖 Claude Code Agent

**Linear Issue:** [${issue.identifier}](${(issue as any).url})

${issue.description ? `\n### Issue Description\n${issue.description}\n` : ""}

### Changes Made

${result.commits && result.commits.length > 0 ? result.commits.map(c => `- ${c.message}`).join("\n") : "See commits for details"}
