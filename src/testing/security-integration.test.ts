...
describe("Security Integration Tests", () => {
  ...
  describe("End-to-End Security Flow", () => {
    ...
    it("should validate session IDs correctly", async () => {
      ...
      expect(SecurityUtils.isValidSessionId("<script>alert(1)</script>")).toBe(
        false,
      );
    ...
    it("should validate commands and paths correctly", async () => {
      ...
      expect(securityValidator.validateCommand("sudo rm -rf /").valid).toBe(
        false,
      );
      ...
      expect(
        securityValidator.validatePath("/home/user/project/src/app.js").valid,
      ).toBe(true);

      // Invalid paths
      expect(securityValidator.validatePath("/etc/passwd").valid).toBe(false);
      expect(securityValidator.validatePath("../../../etc/passwd").valid).toBe(
        false,
      );
    });
