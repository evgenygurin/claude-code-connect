...
export class TestingAgent {
  ...
  async analyzeCoverage(): Promise<TestCoverageAnalysis> {
    ...
    try {
      ...
      const missingTests = sourceFiles.filter(file => !testedFiles.includes(file));
      ...
      const coverage: TestCoverageAnalysis = {
        ...
        coveragePercentage: Math.round((testedFiles.length / sourceFiles.length) * 100),
        missingTests,
   ...
   */
  async generateRecommendations(files: string[]): Promise<TestRecommendation[]> {
    ...
    for (const file of files) {
      try {
        ...
        this.logger.warn(`Failed to analyze file ${file}`, { error: (error as Error).message });
      }
    }
   ...
   */
  async generateSampleTest(recommendation: TestRecommendation): Promise<string> {
    ...
    if (isClass) {
      testContent += this.generateClassTestTemplate(componentName, scenarios, analysis);
    } else {
      testContent += this.generateFunctionTestTemplate(componentName, scenarios, analysis);
    }
   ...
   */
  private mapTestsToSource(sourceFiles: string[], testFiles: string[]): string[] {
  ...
  private async analyzeCodeFile(filePath: string): Promise<CodeAnalysis> {
    ...
    const asyncFunctions = asyncMatches.map(match => match.replace(/async\s+/, ""));
   ...
   */
  private createRecommendation(filePath: string, analysis: CodeAnalysis): TestRecommendation | null {
  ...
  private calculatePriority(filePath: string, analysis: CodeAnalysis): number {
    ...
    if (filePath.includes("session") || filePath.includes("manager")) priority += 3;
    if (filePath.includes("webhook") || filePath.includes("handler")) priority += 3;
    if (filePath.includes("executor") || filePath.includes("claude")) priority += 2;
    if (filePath.includes("storage") || filePath.includes("client")) priority += 2;
  ...
  private getPriorityReason(filePath: string, analysis: CodeAnalysis): string {
    ...
    return reasons.length > 0 ? reasons.join(", ") : "Standard component testing";
  }
   ...
   */
  private generateTestFileHeader(targetFile: string, componentName: string): string {
...
`;
   ...
   */
  private generateClassTestTemplate(className: string, scenarios: TestScenario[], analysis: CodeAnalysis): string {
...
`;
   ...
   */
  private generateFunctionTestTemplate(functionName: string, scenarios: TestScenario[], analysis: CodeAnalysis): string {
